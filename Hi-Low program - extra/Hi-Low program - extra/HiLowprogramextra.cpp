...
void loopClass(int input, int randomInput, int fir, int las, string name) {
	int tries = 0;
	do {
...
int main()
{
	...
	inputFirst = rand() % 100;
	cout << inputFirst << endl;
	cout << "Please give me a number you wish end" << endl;
	
	do {
		inputLast = rand() % 100;
	} while (inputFirst > inputLast); {
		cout << "The number lower than first number, I will creat new one" << endl;
		inputLast = rand() % 100;
	}
	cout << inputLast;
	...
	loopClass(inputNumber, randomNumber, inputFirst, inputLast, name);
	counter(tries);
